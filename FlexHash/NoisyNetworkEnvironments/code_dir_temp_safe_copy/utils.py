...
def get_dataset():
    ...
    if experiment_type == 1 and c_uc == 1:
        ...
        if c_uc == 1:
            ...
            name = (
                f"SimHash-{device_selection}-{accum}-win_{window}-combo_{combo}-cleaned"
            )
        else:
...
def get_dataset_parameterized_auto_select(
    noise, noise_c_uc, device, device_c_uc, device_i_ni, accum, window, combo
):
    if accum == 128:
        names = [
            "dim1",
    ...
    if not noise in [1, 2, 3, 4]:
        raise ValueError(
            "'noise' parameter must be one of the following values: 1, 2 or 3. 1 represents random, 2 "
    ...
    name = (
        f"SimHash-{device_selection}_{device_c_uc_selection}-{noise_name}-"
