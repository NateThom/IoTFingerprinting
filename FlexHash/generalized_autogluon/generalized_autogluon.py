...
def plot_confusion_matrix(confusion_matrix_df, save_path):
    ...
    ax = sns.heatmap(
        confusion_matrix_df, annot=True, fmt="2.2f", cmap="crest", cbar=False
    )
    for t in ax.texts:
            ...
            for combo in range(2, window + 1):
                ...
                if config["downsample"]["sample_ratio"] == "SelectMaxN":
                    if not os.path.isdir(
                        f"./{config['name']}/results/{accumulator}_{window}_{combo}_{config['model_quality']}_stratify-{config['downsample']['stratify_by']}-{config['downsample']['sample_ratio']}-{config['downsample']['MaxN']}"
                    ):
                        os.mkdir(
                            f"./{config['name']}/results/{accumulator}_{window}_{combo}_{config['model_quality']}_stratify-{config['downsample']['stratify_by']}-{config['downsample']['sample_ratio']}-{config['downsample']['MaxN']}"
                        )
                    ...
                    if not os.path.isdir(
                        f"./{config['name']}/results/{accumulator}_{window}_{combo}_{config['model_quality']}_stratify-{config['downsample']['stratify_by']}-{config['downsample']['sample_ratio']}"
                    ):
                        os.mkdir(
                            f"./{config['name']}/results/{accumulator}_{window}_{combo}_{config['model_quality']}_stratify-{config['downsample']['stratify_by']}-{config['downsample']['sample_ratio']}"
                        )
                ...
                for class_index in range(len(config["data"]["classes"])):
                    ...
                    csv_path = (
                        outer + inner + f"{device}_{accumulator}_{window}_{combo}.csv"
                    )
                ...
                for sublist in csv_column_names_list[1:]:
                    if len(sublist) != first_list_len:
                        print(
                            "Error! Not all of the classes defined in the configuration have the same accumulator length."
                        )
                        exit(0)
                ...
                for query_string in label_list:
                    dataset.loc[
                        dataset[dataset["class"].str.startswith(query_string)].index,
                        "class",
                    ] = query_string

                print(
                    f"*** Total samples in {config['name']}_{accumulator}_{window}_{combo}: {len(dataset.index)} ***"
                )
                ...
                if sampling_ratio == "minimum":
                    ...
                    for unique_class in dataset["class"].unique():
                        unique_class_sample_count = dataset[
                            dataset["class"] == unique_class
                        ].shape[0]
                    ...
                    sample_sizes = (
                        dataset[stratify_by]
                        .value_counts()
                        .apply(lambda x: min(x, min_count))
                    ).astype(int)
                    ...
                    sample_sizes = (
                        dataset[stratify_by]
                        .value_counts()
                        .apply(lambda count: min(MaxN, count))
                    )
                else:
                    # Calculate the number of samples to take from each category
                    sample_sizes = (
                        dataset[stratify_by].value_counts() * sampling_ratio
                    ).astype(int)

                # Perform stratified sampling
                dataset = dataset.groupby(stratify_by, group_keys=False).apply(
                    lambda x: x.sample(sample_sizes.loc[x.name])
                )
                ...
                if config["mode"] == "LoadAndTest":
                    test_dataset_df = pd.read_csv(
                        f"./{config['load_root']}/results/{accumulator}_{window}_{combo}_{config['load_model_quality']}_stratify-{config['load_downsample']['stratify_by']}-{config['load_downsample']['sample_ratio']}/test_data.csv"
                    )
                    ...
                    print("\n##########\n")
                    print(
                        "Summary of class variable in test dataset: \n",
                    ...
                    print("\n##########\n")
                    print(
                        "Summary of class variable in train dataset: \n",
                ...
                eval = predictor.evaluate(
                    test_dataset_td, detailed_report=True, silent=True
                )
                ...
                if config["downsample"]["sample_ratio"] == "SelectMaxN":
                    plot_confusion_matrix(
                        ...
                        f"./{config['name']}/results/{accumulator}_{window}_{combo}_{config['model_quality']}_stratify-{config['downsample']['stratify_by']}-{config['downsample']['sample_ratio']}-{config['downsample']['MaxN']}/confusionMatrix",
                    )
                else:
                    plot_confusion_matrix(
                        eval["confusion_matrix"],
                        f"./{config['name']}/results/{accumulator}_{window}_{combo}_{config['model_quality']}_stratify-{config['downsample']['stratify_by']}-{config['downsample']['sample_ratio']}/confusionMatrix",
                    )
                ...
                output_df = pd.DataFrame(
                    np.array(output_list).reshape((1, -1)),
                    columns=output_list_column_names,
                )
                if config["downsample"]["sample_ratio"] == "SelectMaxN":
                    output_df.to_csv(
                        f"./{config['name']}/results/{accumulator}_{window}_{combo}_{config['model_quality']}_stratify-{config['downsample']['stratify_by']}-{config['downsample']['sample_ratio']}-{config['downsample']['MaxN']}/metrics.csv",
                        index=False,
                    )
                    train_dataset_df.to_csv(
                        f"./{config['name']}/results/{accumulator}_{window}_{combo}_{config['model_quality']}_stratify-{config['downsample']['stratify_by']}-{config['downsample']['sample_ratio']}-{config['downsample']['MaxN']}/train_data.csv",
                        index=False,
                    )
                    test_dataset_df.to_csv(
                        f"./{config['name']}/results/{accumulator}_{window}_{combo}_{config['model_quality']}_stratify-{config['downsample']['stratify_by']}-{config['downsample']['sample_ratio']}-{config['downsample']['MaxN']}/test_data.csv",
                        index=False,
                    )
                else:
                    output_df.to_csv(
                        f"./{config['name']}/results/{accumulator}_{window}_{combo}_{config['model_quality']}_stratify-{config['downsample']['stratify_by']}-{config['downsample']['sample_ratio']}/metrics.csv",
                        index=False,
                    )
                    train_dataset_df.to_csv(
                        f"./{config['name']}/results/{accumulator}_{window}_{combo}_{config['model_quality']}_stratify-{config['downsample']['stratify_by']}-{config['downsample']['sample_ratio']}/train_data.csv",
                        index=False,
                    )
                    test_dataset_df.to_csv(
                        f"./{config['name']}/results/{accumulator}_{window}_{combo}_{config['model_quality']}_stratify-{config['downsample']['stratify_by']}-{config['downsample']['sample_ratio']}/test_data.csv",
                        index=False,
                    )
