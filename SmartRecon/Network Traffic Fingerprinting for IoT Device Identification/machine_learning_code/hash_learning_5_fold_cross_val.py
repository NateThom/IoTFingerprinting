...
def setup_menu():
    ...
    dataset_list = [
        ["../data/batyr_modified/", "Batyr Modified"],
        ["../data/batyr_modified_categories/", "Batyr Modified Categories"],
        ["../data/hashes_cleaned/", "Cleaned Hashes"],
        ["../data/hashes_uncleaned", "Uncleaned Hashes"],
...
def per_device(dataset, labels_numeric, class_label_dict):
    ...
    print(
        len(x[device_name]["test"]),
        len(y[device_name]["test"]),
        len(x["train"]),
        len(y["train"]),
    )
    ...
    for model_name, model in models:
        ...
        for dev_index, device_name in enumerate(labels_numeric):
            ...
            wandb.log(
                {
                    f"{class_label_dict[device_name]} accuracy SR on {name_of_current_data}": accuracy,
                    "Dataset": dataset_dict[dataset_name],
                    "Num Samples": dataset.shape[0],
                }
            )
            wandb.log(
                {
                    f"{class_label_dict[device_name]} precision SR on {name_of_current_data}": precision,
                    "Dataset": dataset_dict[dataset_name],
                    "Num Samples": dataset.shape[0],
                }
            )
            wandb.log(
                {
                    f"{class_label_dict[device_name]} recall SR on {name_of_current_data}": recall,
                    "Dataset": dataset_dict[dataset_name],
                    "Num Samples": dataset.shape[0],
                }
            )
            wandb.log(
                {
                    f"{class_label_dict[device_name]} f1 SR on {name_of_current_data}": f1,
                    "Dataset": dataset_dict[dataset_name],
                    "Num Samples": dataset.shape[0],
                }
            )
        ...
        wandb.log(
            {
                f"Total accuracy SR on {name_of_current_data}": total_accuracy,
                "Dataset": dataset_dict[dataset_name],
                "Num Samples": dataset.shape[0],
            }
        )
        wandb.log(
            {
                f"Total precision SR on {name_of_current_data}": total_precision,
                "Dataset": dataset_dict[dataset_name],
                "Num Samples": dataset.shape[0],
            }
        )
        wandb.log(
            {
                f"Total recall SR on {name_of_current_data}": total_recall,
                "Dataset": dataset_dict[dataset_name],
                "Num Samples": dataset.shape[0],
            }
        )
        wandb.log(
            {
                f"Total f1 SR on {name_of_current_data}": total_f1,
                "Dataset": dataset_dict[dataset_name],
                "Num Samples": dataset.shape[0],
            }
        )
...
path_to_csvs, name_of_current_data, print_num_samples_bool, prototyping_bool = (
    setup_menu()
)
