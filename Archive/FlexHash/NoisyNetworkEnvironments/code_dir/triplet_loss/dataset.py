...
class Triplet_Dataset(Dataset):
    def __init__(self, args, fold):
        ...
        self.data = pd.read_csv(
            args.dataset_dir + args.dataset + csv_filename,
        ...
        for device_name in labels_numeric:
            ...
            if fold == "training":
                self.dataset = pd.concat(
                    [
                        self.dataset,
                        current_device_dataset[
                            int(current_device_dataset_length * 0.2) :
                        ],
                    ]
                )
                ...
                self.dataset = pd.concat(
                    [
                        self.dataset,
                        current_device_dataset[
                            int(current_device_dataset_length * 0.1) :
                        ],
                    ]
                )
            elif fold == "testing":
                self.dataset = pd.concat(
                    [
                        self.dataset,
                        current_device_dataset[
                            int(current_device_dataset_length * 0.1) : int(
                                current_device_dataset_length * 0.2
                            )
                        ],
                    ]
                )
        ...
        self.dataset_no_class = torch.from_numpy(
            self.dataset.drop("class", axis=1).values.astype("float32")
        )
        ...
        for device_name in labels_numeric:
            ...
            self.dataset_by_class[device_name] = [
                torch.from_numpy(entry.drop("class", axis=1).values.astype("float32")),
                len(entry.index) - 1,
            ]
    ...
    def __getitem__(self, idx):
        ...
        neg_class_choices = [
            choice
            for choice in self.dataset_by_class.keys()
            if choice not in [anchor_class]
        ]
