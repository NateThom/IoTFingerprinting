...
class TripletLearner(LightningModule):
    def __init__(self, args):
        ...
        self.model = MLP(
            ...
            self.args.num_layers_hidden_layers,
            self.args.num_nodes_per_layer,
            activation="ReLU",
            dropout=0.5,
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        anchor_out, positive_out, negative_out = self.forward(
    ...
    def training_epoch_end(self, outputs):
        ...
        for output in outputs:
            if math.isnan(output["loss"].item()):
                print("NaN")
        ...
        print(f"Training Loss Epoch: {average_loss}")
...
if __name__ == "__main__":
    ...
    if args.reload:
        tl = TripletLearner.load_from_checkpoint(
            args.model_path + args.model_file, args=args
        )
    else:
    ...
    if args.save == True:
        checkpoint_callback = ModelCheckpoint(
            ...
            filename="{epoch:02d}-{Training Loss:.05f}-"
            + f"{args.learning_rate}-"
            + f"{args.epochs}-"
            + f"{args.num_hidden_layers}-"
            + f"{args.num_nodes_per_layer}",
            save_top_k=1,
            mode="min",
        )
